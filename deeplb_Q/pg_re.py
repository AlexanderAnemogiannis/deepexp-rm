...
### return NN parameters for policy gradient learner/agent
...
def get_traj(agent, env, episode_max_length):
    ...
    for _ in xrange(episode_max_length):
        ...
        ### get prob. dist over actions
        act_prob = agent.get_one_act_prob(ob)

        ### draw one-hot vector (action) from distribution
...
def concatenate_all_ob(trajs, pa):
    """ 
    Returns array of observations (images) across all trajectories
    and timesteps within those trajectories
    """
    timesteps_total = 0
    ...
    ### iterate over all trajectories, timesteps in trajectories
    timesteps = 0
    for i in xrange(len(trajs)):
